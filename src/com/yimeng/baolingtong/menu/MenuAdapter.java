    ...
    private void setViews() {
        for (int i = 0; i < mMenuObjects.size(); i++) {
            ...
            mMenuWrapper.addView(Utils.getImageWrapper(mContext, menuObject, mMenuItemSize,
                    clickItem, longClickItem, false, MenuManager.getMenuItemStatus(i)));
//            i != mMenuObjects.size() - 1
    ...
    private void viewClicked(View v) {
    	int id = v.getId();
    	MenuManager.changeItemStatus(id);
    	int resId = MenuManager.getMenuIconResource(id);
    	((ImageView)v).setImageResource(resId);
    	mOnItemClickListenerCalled.onClick(mClickedView, id, MenuManager.getMenuItemStatus(id));
